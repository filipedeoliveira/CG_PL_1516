...
void drawCylinder(float raio, float altura, float lados) {
	float alfa = (2 * M_PI) / lados, angulo = 0.0;
	...
	for (int i = 0; i < lados; i++) {
		//base inferior
		glColor3f(0, 0, 1);
		glVertex3f(raio*sin(angulo + alfa), 0, raio*cos(angulo + alfa));
		glVertex3f(raio*sin(angulo), 0, raio*cos(angulo));
		glVertex3f(0, 0, 0);
